...
def inline_text(value):
    """Escape transcript text and render the editor's simple *term* convention."""
    escaped = html.escape(value)
    parts = escaped.split("*")
    return "".join(f"<em>{part}</em>" if i % 2 else part for i, part in enumerate(parts))
...
for row in rows:
    ...
    family_note = row.get("family_note", "").strip()
    note = f'<span class="editorial-note">Family note: {html.escape(family_note)}</span>' if family_note else ""
    key = f'{row["start"]:.3f}'
    article.append(
        ...
        f'<div class="utterance-main"><p>{badge}{speaker}<span class="transcript-text">{inline_text(row["text"])}</span>{note}</p>'
...
#youtubePlayer{{display:block;width:100%;height:auto;aspect-ratio:16/9;border:0;background:#111}}.player.compact #youtubePlayer{{height:160px;aspect-ratio:auto}}.player.large #youtubePlayer{{height:min(68vh,620px);aspect-ratio:auto}}.player-meta{{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-top:8px}}.player p{{margin:0;color:var(--muted);font-size:12px}}.size-controls{{display:flex;gap:4px;align-items:center;color:var(--muted);font-size:11px}}.size-controls button{{border:1px solid var(--rule);background:white;color:var(--ink);padding:5px 8px;border-radius:3px;font:11px system-ui}}.size-controls button.selected{{background:var(--ink);color:white}}.toolbar{{display:flex;flex-wrap:wrap;gap:12px;margin:18px 0}}input[type=search]{{flex:1;min-width:220px;padding:10px;border:1px solid var(--rule);background:#fff}}
...
<div class="player compact" id="playerPanel"><iframe id="youtubePlayer" title="The Abe Tabak Tapes — Tape 1" src="https://www.youtube.com/embed/fJDnT9XBD58?enablejsapi=1&amp;playsinline=1&amp;rel=0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe><div class="player-meta"><p id="playerStatus">Loading YouTube player…</p><div class="size-controls" aria-label="Player size"><span>Player size</span><button type="button" data-player-size="compact">Compact</button><button type="button" data-player-size="standard">Standard</button><button type="button" data-player-size="large">Large</button></div></div></div>
...
<section id="transcript"><div class="review-tools"><strong>Family review workspace</strong><br>Use “Correct or annotate” beside any passage. Changes save only in this browser. Export the correction file when you are ready to share it; importing a previous export merges it into this browser’s saved work.<div class="review-actions"><button id="exportEdits" type="button">Export corrections (.json)</button><button id="exportTranscript" type="button">Export transcript for Word (.rtf)</button><label class="import-label">Import corrections<input id="importEdits" type="file" accept="application/json,.json"></label><span id="editCount"></span></div></div><div class="toolbar"><input id="search" type="search" placeholder="Search the transcript…"><label><input id="review" type="checkbox"> Check audio only</label><label><input id="followPlayback" type="checkbox" checked> Follow playback</label></div><div id="count"></div>{''.join(article)}</section>
<section id="notes" class="notes" hidden>{notes}</section>
</main></div><script>
const rows=[...document.querySelectorAll('.utterance')],status=document.getElementById('playerStatus'),storageKey='abe-tabak-tape1-corrections-v1',speakerOptions=['Abe','Bella','Nina','Len','Dan','Multiple speakers','Unclear speaker'];let player=null,playerReady=false,pendingSeek=null,playbackStopAt=null,corrections={{}};
const playerPanel=document.getElementById('playerPanel'),playerSizeKey='abe-tabak-tape1-player-size-v1';
function setPlayerSize(size){{if(!['compact','standard','large'].includes(size))size='compact';playerPanel.classList.remove('compact','large');if(size!=='standard')playerPanel.classList.add(size);document.querySelectorAll('[data-player-size]').forEach(button=>button.classList.toggle('selected',button.dataset.playerSize===size));localStorage.setItem(playerSizeKey,size);}}
document.querySelectorAll('[data-player-size]').forEach(button=>button.onclick=()=>setPlayerSize(button.dataset.playerSize));setPlayerSize(localStorage.getItem(playerSizeKey)||'compact');
try{{corrections=JSON.parse(localStorage.getItem(storageKey)||'{{}}');}}catch(e){{corrections={{}};}}
function originalFor(row){{return{{start:+row.dataset.start,end:+row.dataset.end,text:row.querySelector('.transcript-text').textContent,speaker:row.querySelector('.speaker').textContent.replace(/:$/,''),note:row.querySelector('.editorial-note')?.textContent.replace(/^Family note: /,'')||''}};}}
...
Object.entries(corrections).forEach(([key,edit])=>{{const base=originals[key];if(base&&edit.speaker===base.speaker&&String(edit.text||'').replaceAll('*','')===base.text&&String(edit.note||'').trim()===base.note)delete corrections[key];}});
function renderTranscript(target,value){{target.replaceChildren();String(value).split('*').forEach((part,index)=>{{if(index%2){{const em=document.createElement('em');em.textContent=part;target.appendChild(em);}}else target.appendChild(document.createTextNode(part));}});}}
function applyCorrection(row){{const edit=corrections[row.dataset.key],base=originals[row.dataset.key];row.querySelector('.speaker').textContent=(edit?.speaker||base.speaker)+':';renderTranscript(row.querySelector('.transcript-text'),edit?.text||base.text);row.classList.toggle('corrected',!!edit);let note=row.querySelector('.correction-note');if(edit?.note){{if(!note){{note=document.createElement('span');note.className='correction-note';row.querySelector('p').appendChild(note);}}note.textContent='Family note: '+edit.note;}}else if(note)note.remove();}}
rows.forEach(applyCorrection);saveState();
function closeEditor(editor){{editor.remove();if(!document.querySelector('.editor'))playbackStopAt=null;}}
function limitPlaybackTo(row){{playbackStopAt=+row.dataset.end;if(playerReady&&typeof player.getCurrentTime==='function'&&player.getCurrentTime()>=playbackStopAt)player.pauseVideo();}}
function openEditor(row){{let editor=row.querySelector('.editor');if(editor){{closeEditor(editor);return;}}limitPlaybackTo(row);if(playerReady)player.pauseVideo();const key=row.dataset.key,base=originals[key],edit=corrections[key]||{{}};editor=document.createElement('div');editor.className='editor';const speaker=document.createElement('select');speakerOptions.forEach(name=>{{const option=document.createElement('option');option.value=option.textContent=name;speaker.appendChild(option);}});speaker.value=edit.speaker||base.speaker;const text=document.createElement('textarea');text.className='edit-text';text.value=edit.text||base.text;const note=document.createElement('textarea');note.className='edit-note';note.placeholder='Names, context, uncertainty, translation, or anything else to tell the editor';note.value=edit.note||'';const speakerLabel=document.createElement('label');speakerLabel.textContent='Speaker';speakerLabel.appendChild(speaker);const textLabel=document.createElement('label');textLabel.textContent='Corrected transcript';textLabel.appendChild(text);const noteLabel=document.createElement('label');noteLabel.textContent='Family note';noteLabel.appendChild(note);const controls=document.createElement('div');controls.className='editor-buttons';const close=document.createElement('button');close.type='button';close.textContent='Close';close.onclick=()=>closeEditor(editor);const discard=document.createElement('button');discard.type='button';discard.textContent='Discard saved correction';discard.onclick=()=>{{delete corrections[key];saveState();applyCorrection(row);closeEditor(editor);}};const saved=document.createElement('span');saved.className='autosave';saved.textContent='Changes autosave';controls.append(close,discard,saved);editor.append(speakerLabel,textLabel,noteLabel,controls);function update(){{const next={{start:base.start,end:base.end,original_speaker:base.speaker,original_text:base.text,speaker:speaker.value,text:text.value,note:note.value,updated_at:new Date().toISOString()}};const unchanged=next.speaker===base.speaker&&next.text===base.text&&!next.note.trim();if(unchanged)delete corrections[key];else corrections[key]=next;saveState();applyCorrection(row);}}speaker.onchange=update;text.oninput=update;note.oninput=update;row.querySelector('.utterance-main').appendChild(editor);}}
...
function rtfText(value){{let out='';for(let i=0;i<String(value).length;i++){{const code=String(value).charCodeAt(i),char=String.fromCharCode(code);if(char==='\\\\'||char==='{{'||char==='}}')out+='\\\\'+char;else if(code===10)out+='\\\\line ';else if(code===13){{}}else if(code<32)out+=' ';else if(code>126)out+='\\\\u'+(code>32767?code-65536:code)+'?';else out+=char;}}return out;}}
document.getElementById('exportTranscript').onclick=()=>{{const parts=['{{\\\\rtf1\\\\ansi\\\\deff0\\\\uc1{{\\\\fonttbl{{\\\\f0 Calibri;}}}}\\\\f0\\\\fs22'];parts.push('\\\\b\\\\fs32 '+rtfText('Abe Tabak Tapes — Tape 1')+'\\\\b0\\\\fs22\\\\par\\\\par');document.querySelectorAll('.chapter').forEach(chapter=>{{const heading=chapter.querySelector('h2'),title=heading.cloneNode(true);title.querySelector('small')?.remove();parts.push('\\\\b\\\\fs26 '+rtfText(title.textContent.trim())+'\\\\b0\\\\fs22\\\\par');chapter.querySelectorAll('.utterance').forEach(row=>{{const stamp=row.querySelector('.stamp')?.textContent.trim()||'',speaker=row.querySelector('.speaker')?.textContent.trim()||'',transcript=row.querySelector('.transcript-text')?.textContent.trim()||'',check=row.classList.contains('review')?' [Check audio]':'';parts.push('\\\\i '+rtfText(stamp)+'\\\\i0  \\\\b '+rtfText(speaker)+'\\\\b0 '+rtfText(transcript+check)+'\\\\par');}});parts.push('\\\\par');}});parts.push('}}');const blob=new Blob([parts.join('')],{{type:'application/rtf'}}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='Abe_Tabak_Tape_1_transcript.rtf';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}};
...
document.querySelectorAll('[data-seek]').forEach(x=>x.onclick=e=>{{if(x.tagName==='A'){{e.preventDefault();const chapter=document.querySelector(x.getAttribute('href'));if(chapter)chapter.scrollIntoView({{behavior:'smooth',block:'start'}});}}const editorRow=document.querySelector('.editor')?.closest('.utterance');if(editorRow&&x.closest('.utterance'))limitPlaybackTo(editorRow);else playbackStopAt=null;seek(x.dataset.seek);}});
let activeRow=null;
function editingTranscript(){{const focused=document.activeElement;return !!focused?.closest?.('.editor');}}
function followRow(row){{if(!document.getElementById('followPlayback').checked||editingTranscript()||document.getElementById('transcript').hidden)return;const box=row.getBoundingClientRect(),playerBox=document.querySelector('.player').getBoundingClientRect(),top=Math.min(window.innerHeight-90,Math.max(16,playerBox.bottom+16)),bottom=Math.max(top+70,window.innerHeight-24);if(box.top<top||box.bottom>bottom)window.scrollBy({{top:box.top-top,behavior:'smooth'}});}}
setInterval(()=>{{if(!playerReady||typeof player.getCurrentTime!=='function')return;const t=player.getCurrentTime();if(playbackStopAt!==null&&t>=playbackStopAt)player.pauseVideo();const next=rows.find(r=>!r.hidden&&t>=+r.dataset.start&&t<+r.dataset.end)||null;if(next===activeRow)return;if(activeRow)activeRow.classList.remove('active');activeRow=next;if(activeRow){{activeRow.classList.add('active');if(typeof player.getPlayerState!=='function'||player.getPlayerState()===YT.PlayerState.PLAYING)followRow(activeRow);}}}},100);
