...
function rtfText(value) {
  ...
  for (let i = 0; i < String(value).length; i++) {
    const code = String(value).charCodeAt(i), char = String.fromCharCode(code);
...
document.getElementById("exportTranscript").onclick = () => {
  const parts = ["{\\rtf1\\ansi\\deff0\\uc1{\\fonttbl{\\f0 Calibri;}}\\f0\\fs22"];
  const title = document.querySelector("h1").textContent.trim() + " — " +
    config.export_recording;
  parts.push("\\b\\fs32 " + rtfText(title) + "\\b0\\fs22\\par\\par");
  ...
  a.href = url;
  a.download = config.export_filename.replace("_family_corrections.json", "_transcript.rtf");
  a.click();
  setTimeout(() => URL.revokeObjectURL(url), 1000);
};
